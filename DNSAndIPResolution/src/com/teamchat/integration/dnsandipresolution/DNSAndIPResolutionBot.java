...
public class DNSAndIPResolutionBot
{
	...
	public void methodCheck(TeamchatAPI api) throws Exception
	{
		...
		if (method.equals("Get Hostname for IP address"))
		{
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the IP address" + Utility.paraEnd).setReplyScreen(f).setReplyLabel("Enter")
		...
		} else if (method.equals("Get IP address for DNS address"))
		{
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the DNS address" + Utility.paraEnd).setReplyScreen(f).setReplyLabel("Enter")
		...
		} else if (method.equals("Check if port is open or not"))
		{
			...
			api.perform(api
					.context()
					.currentRoom()
					.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the Host name and the port number" + Utility.paraEnd).setReplyScreen(f)
	...
	public void IP2DNS(TeamchatAPI api)
	{
		...
		api.perform(api
				.context()
				.currentRoom()
				.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the IP address" + Utility.paraEnd).setReplyScreen(f).setReplyLabel("Enter").showDetails(true)
	...
	{ 
		...
		api.perform(api
				.context()
				.currentRoom()
				.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the DNS address" + Utility.paraEnd).setReplyScreen(f).setReplyLabel("Enter").showDetails(true)
	...
	public void CHKPRT(TeamchatAPI api)
	{
		...
		api.perform(api
				.context()
				.currentRoom()
				.post(new PrimaryChatlet().setQuestionHtml(Utility.image + Utility.paraStart + "Please enter the Host name and Port number" + Utility.paraEnd).setReplyScreen(f)
						.setReplyLabel("Enter").showDetails(true).setDetailsLabel("replies").allowComments(true).alias("checkport")));
	}
	...
	public void ipAddr(TeamchatAPI api) throws Exception
	{
		...
		if (responseCode == 200)
		{
			...
			if (json.has("host") && !json.isNull("host"))
