...
public class RecipeBot
{
	...
	public void help(TeamchatAPI api)
	{
		...
		f.addField(api.objects().input().label("Enter a keyword for search, for eg. Pasta").name("search"));
		api.perform(api.context().currentRoom().post(new PrimaryChatlet().setQuestionHtml(Utility.help).setReplyScreen(f).setReplyLabel("Enter").alias("search")));
	}
		...
		if (responseCode == 200)
		{
			...
			if (count != 0)
			{
				...
				for (int i = 0; i<jsona.length() && i<5; i++)
				{
					...
					recipes.append("<p style='margin-top:5px; padding-right:5px; padding-left:5px'><b>Source: <a href='" + pubUrl + "' target='_blank'>" + pubUrl + "</a></b>" 
									+ "<ul>"
									+ "<li>" + title + "</li>"
									+ "<li><a href='" + sourceUrl + "' target='_blank'>Click here to view</a></li>"
									+ "</ul>"
									+ "<br/><br/>");
				}
				recipes.append("</div>");
