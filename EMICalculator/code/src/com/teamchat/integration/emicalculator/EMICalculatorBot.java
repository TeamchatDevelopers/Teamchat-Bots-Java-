...
public class EMICalculatorBot
{
	...
	public void help(TeamchatAPI api)
	{
		...
		api.perform(api.context().currentRoom()
				.post(new PrimaryChatlet().setQuestionHtml(Utility.help).setReplyScreen(f).setReplyLabel("Enter").alias("emi")));
	}
		...
		public void calcEmiAllMonths(double p, double r, double n, TeamchatAPI api)
		{
			...
			api.perform(api
					...
					.post(new PrimaryChatlet()
							.setQuestionHtml("<center><img src='http://integration.teamchat.com/sol/bot-images/emi.jpg' width='150' /></center><p style='padding-right:5px; padding-left:5px; margin-top:5px'><b>Principal: </b>"
									+ P
									+ "<br/><b>Rate of Interest: </b>"
									+ r
									+ "<br/><b>Number of Months: </b>"
									+ (int) n
									+ "<br/><b>EMI: </b>"
									+ Math.round(e)
									+ "<br/><b>Total Interest: </b>"
									+ totalInt + "<br/><b>Total Amount: </b>" + totalAmt + "</p>")));
			...
			for (double i = 1; i <= n; i++)
			{
				...
				if (smoke == 0)
				{
					emi.append(Utility.rowWhite.replace("__aa", (int) i + "").replace("__bb", Math.round(intPerMonth) + "").replace("__cc", Math.round((e) - intPerMonth) + "")
							.replace("__dd", Math.round(P) + ""));
					smoke = 1;
				}
				else
				{
					emi.append(Utility.rowSmoke.replace("__aa", (int) i + "").replace("__bb", Math.round(intPerMonth) + "").replace("__cc", Math.round((e) - intPerMonth) + "")
							.replace("__dd", Math.round(P) + ""));
					smoke = 0;
				}
			}
