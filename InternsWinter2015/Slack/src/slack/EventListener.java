...
public class EventListener {
	...
	public TeamchatAPI api1; //assigned as soon as keyword 'slack' in entered by user

	public void setAPI(TeamchatAPI api)
	{
		this.api1 = api;
	}
	
	public void listenForEvents(String token) throws Exception {
		...
		session.addMessagePostedListener(new SlackMessagePostedListener() {
			...
			public void onEvent(SlackMessagePosted event, SlackSession session) {
				...
				if(!event.getMessageContent().endsWith("has joined the channel")){
				api1.performPostInCurrentRoom(new TextChatlet(event.getSender().getUserName()+": \""+event.getMessageContent()+"\" in channel "+event.getChannel().getName()));
				}
				}
		});
		...
		session.addchannelCreatedListener(new SlackChannelCreatedListener() {
			...
			public void onEvent(SlackChannelCreated event, SlackSession session) {
				...
				api1.performPostInCurrentRoom(new TextChatlet(event.getCreator().getUserName()+" joined #"+event.getSlackChannel().getName()));
				//System.err.println(event.getCreator().getUserName()+" joined #"+event.getSlackChannel().getName());
